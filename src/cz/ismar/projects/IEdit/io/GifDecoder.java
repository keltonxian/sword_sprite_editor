...
public class GifDecoder
{
	...
	protected static final int MaxStackSize = 4096; // max decoder pixel stack
	// size
	...
	protected int[] getPixels()
	{
		...
		int[] dest = new int[width * height];// ((DataBufferInt)
		...
		if(lastDispose > 0)
		{
			...
			{
				int[] prev = lastImage;
				System.arraycopy(prev, 0, dest, 0, width * height); // copy
				// pixels
				...
				if(lastDispose == 2)
				{
					...
					if(!transparency)
						c = encodeColor(new Color(lastBgColor)); // use given
					// background
					// color
