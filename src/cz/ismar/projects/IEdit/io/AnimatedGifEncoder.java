...
public class AnimatedGifEncoder
{
	...
	private void addImage(BufferedImage im) throws IOException
	{
		...
		System.out.println("analyzePixels took:" + (System.currentTimeMillis() - time) + " ms");
		time = System.currentTimeMillis();
		if(firstFrame)
		{
		...
		firstFrame = false;
		System.out.println("writing out data took:" + (System.currentTimeMillis() - time) + " ms");
	}
	 ...
	 */
	private static final void analyzePixels(AnimatedGifEncoder animatedGifEncoder)
	{
		int len = animatedGifEncoder.pixels.length;
		int nPix = len / 3;
		animatedGifEncoder.indexedPixels = new byte[nPix];
		NeuQuant nq = new NeuQuant(animatedGifEncoder.pixels, len, animatedGifEncoder.sample);
		...
		{
			index = nq.map(animatedGifEncoder.pixels[k++] & 0xff, animatedGifEncoder.pixels[k++] & 0xff, animatedGifEncoder.pixels[k++] & 0xff);
	...
	/**
	 * Returns index of palette color closest to c This is using square error
	 * and a search, for each color. It is not efficient. todo: optimize this
	 * search
	 * 
	 */
	private final static int findClosest(byte[] colorTab1, boolean[] usedEntry1, Color c)
	{
		if(colorTab1 == null)
			return -1;
		int r = c.getRed();
